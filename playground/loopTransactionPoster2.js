...
var testAddresses = require('./testAddresses');
...
var CreateRequest = (async (callback) => {
    ...
    const data = JSON.stringify({
        from: testAddresses.Tommy().Address,
        to: testAddresses.Timmy().Address,
        amount: 2,
        privatekey: testAddresses.Tommy().PrivateKey
    });
    const options = {
        uri: nodeEndpoint,
        method: 'POST',
...
var SubmitRequest = (async (createRequestResults, callback) => {
    ...
    const data = JSON.stringify({
        ...
        to: testAddresses.Timmy().Address,
        amount: 2,
        publickey: testAddresses.Tommy().PublicKey,
